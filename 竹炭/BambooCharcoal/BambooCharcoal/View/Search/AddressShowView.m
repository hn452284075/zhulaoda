...
//        NSDictionary *dic = [NSDictionary dictionaryWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"area" ofType:@"plist"]];
    ...
    NSArray *array = jsonDict[@"params"];
    for (int i=0;i<array.count;i++) {
        NSDictionary *dic = array[i];
        NSArray *proArray = dic[@"childList"];
        NSString *p_name = dic[@"name"];
        NSMutableDictionary *p_dic = [[NSMutableDictionary alloc] init];
        for(int j=0;j<proArray.count;j++)
        {
            NSDictionary *prodic = proArray[j];
            NSArray *disArray = prodic[@"childList"];
            NSString *c_name = prodic[@"name"];
            NSMutableArray *arr_3 = [[NSMutableArray alloc] init];
            for(int k=0;k<disArray.count;k++)
            {
                NSDictionary *disdic = disArray[k];
                NSString *t_name = disdic[@"name"];
                [arr_3 addObject:t_name];
            }
            [p_dic setObject:arr_3 forKey:c_name];
        }
        [resultDic setObject:p_dic forKey:p_name];
    }
    
    
    self.locationModel = [[EWCountryModel alloc]initWithDic:resultDic];
...
//    }
    ...
    [self.delegate adressSelectedPro:self.provinceStr city:self.cityStr area:self.areaStr];
