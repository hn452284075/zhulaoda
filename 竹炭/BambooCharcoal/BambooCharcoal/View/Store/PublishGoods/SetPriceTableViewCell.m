...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        filed1.textColor = kGetColor(0x22, 0x22, 0x22);
        [filed1 addTarget:self action:@selector(textfiledChanged:event:) forControlEvents:UIControlEventEditingChanged];
        ...
        [filed2 addTarget:self action:@selector(textfiledChanged:event:) forControlEvents:UIControlEventEditingChanged];
        ...
        [msglab mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerY.equalTo(self.mas_centerY).offset(0);
            make.left.equalTo(filed1.mas_right).offset(15+100);
            make.width.mas_offset(60);
            make.height.mas_equalTo(15);
        }];
        msglab.textAlignment = NSTextAlignmentLeft;
...
{
    UIImageView *arrowimg = [self viewWithTag:5];
    if(arrowimg.hidden)
        [self.delegate addOrSubFunction:0 row:self.row];
    else
        [self.delegate addOrSubFunction:1 row:self.row];
}


- (void)textfiledChanged:(UITextField *)filed event:(id)event
{
    if(filed.tag == 3)
    {
        self.string1 = filed.text;
    }
    else if(filed.tag == 4)
    {
        self.string2 = filed.text;
    }
    [self.delegate filedTextChanged:self.string1 filed2:self.string2 event:self];
}
