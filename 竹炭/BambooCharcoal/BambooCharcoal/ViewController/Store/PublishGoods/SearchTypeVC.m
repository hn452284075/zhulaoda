...
-(void)_requestStoreData{
    ...
    NSDictionary *dic=@{@"searchKey":_searchField.text};
    
    [AFNHttpRequestOPManager postWithParameters:dic subUrl:@"category/goodsProductSearch" block:^(NSDictionary *resultDic, NSError *error) {
           [weak_self dissmiss];
...
-(void)jumpBack:(NSDictionary *)dic{
    for (UIViewController *controller in self.navigationController.viewControllers) {
        if([controller isKindOfClass:[PublishGoodsVC class]]) {
              [kNotification postNotificationName:@"goodsType" object:dic];
             [self.navigationController popToViewController:controller animated:YES];
        }
        if([controller isKindOfClass:[PublishProcurementVC class]]) {
              [kNotification postNotificationName:@"goodsType" object:dic];
             [self.navigationController popToViewController:controller animated:YES];
        }
    }
...
- (MycommonTableView *)listTableView{
    ...
    if (!_listTableView) {
        ...
        _listTableView.cellHeight = 44.0f;
        
        WEAK_SELF
        [_listTableView configurecellNibName:@"SearchTypeCell" configurecellData:^(UITableView *tableView, id cellModel, UITableViewCell *cell, NSUInteger index) {
            SearchTypeCell *type = (SearchTypeCell *)cell;
            NSString *str = cellModel[@"categoryName"];
            str = [str stringByReplacingOccurrencesOfString:@"/" withString:@" > "];
            type.title.text = [NSString stringWithFormat:@"%@ > %@",str,cellModel[@"name"]];
            
        } clickCell:^(UITableView *tableView, id cellModel, UITableViewCell *cell, NSIndexPath *clickIndexPath) {
            
            [weak_self jumpBack:cellModel];
        }];
