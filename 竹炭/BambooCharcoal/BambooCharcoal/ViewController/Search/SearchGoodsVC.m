...
@interface SearchGoodsVC ()<UICollectionViewDelegate,UICollectionViewDataSource,UITextFieldDelegate>
{
    UIView  *lineView;
}
...
#pragma mark ------------------------Api----------------------------------
#pragma mark ---- 解析热门搜索分类
-(void)requestHotSearchList{
     WEAK_SELF
     [self showHub];
     [AFNHttpRequestOPManager postWithParameters:nil subUrl:@"category/searchTopCategory" block:^(NSDictionary *resultDic, NSError *error) {
         [weak_self dissmiss];
         if ([resultDic[@"code"] integerValue]==200) {
             NSArray *arr = [GoodsCategoryModel mj_objectArrayWithKeyValuesArray:resultDic[@"params"]];
             NSMutableArray *cateArr = [[NSMutableArray alloc] init];
             for(GoodsCategoryModel *cm in arr)
             {
                 [cateArr addObject:cm.cateGoryName];
             }
             NSDictionary *hotDic=@{@"title":@"热门搜索",@"content":cateArr};
             [weak_self.searchArr addObject:hotDic];
