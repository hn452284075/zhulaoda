...
@interface NTESLDMainViewController () <NTESLiveDetectViewDelegate, NTESTimeoutToastViewDelegate,CheckStatusDelegate>
...
- (void)startLiveDetect {
    ...
    [self.detector startLiveDetectWithBusinessID:@"647276853cf44ccdb83e1945e1831a67" actionsHandler:^(NSDictionary * _Nonnull params) {
...
- (void)showToastWithLiveDetectStatus:(NTESLDStatus)status {
    ...
    switch (status) {
        case NTESLDCheckPass:
        {
            ...
            vc.detect_name  = self.nameString;
            vc.detect_card  = self.cardString;
            vc.detect_token = token;
            vc.delegate = self;
        ...
        default:
        {
            [self.delegate NTESDetectResult:0]; //检测不通过
        }
            break;
    }
