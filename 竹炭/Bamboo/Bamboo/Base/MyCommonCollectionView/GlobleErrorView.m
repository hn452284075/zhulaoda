...
@property (nonatomic,strong)GlobleErrorView *nodataView;

@end

@implementation GlobleErrorView
...
+(id)showInContentView:(UIView *)contentView withReloadBlock:(ReloadBlock)reloadBlock requestResultType:(NetWorkRequestResultType)requestResultType
{
    ...
    GlobleErrorView *sel = [self shareNodataView];
...
{
    NSString *alertImageName = @"空数据页状态";
    if (!alertTitle) {
        alertTitle = @"暂无数据";
    }
    if (!alertImageName) {
        alertImageName = @"空数据页状态";
    }
...
+(instancetype)showInContentView:(UIView *)contentView
                 withReloadBlock:(ReloadBlock)reloadBlock
                      alertTitle:(NSString *)alertTitle
                  alertImageName:(NSString *)alertImageName
{
    ...
    return [self _comminSetWithContentView:contentView withReloadBlock:reloadBlock alertTitle:alertTitle alertImageName:alertImageName];
    
}


+(GlobleErrorView *)_comminSetWithContentView:(UIView *)contentView withReloadBlock:(ReloadBlock)reloadBlock  alertTitle:(NSString *)alertTitle alertImageName:(NSString *)alertImageName{
...
+ (GlobleErrorView *)shareNodataView {
    return BoundNibView(@"GlobleErrorView", GlobleErrorView);
}
