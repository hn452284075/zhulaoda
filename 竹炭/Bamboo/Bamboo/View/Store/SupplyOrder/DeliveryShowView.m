...
-(void)initUI{
       ...
       _viewBG.backgroundColor =  [UIColor colorWithWhite:0.f alpha:0.5];
        ...
        UITapGestureRecognizer *recognizer = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(colse)];
       recognizer.delegate = self;
       [_viewBG addGestureRecognizer:recognizer];
...
-(void)determineBtnClick{
    if (isEmpty(self.companyField.text)) {
        [[BaseLoadingView sharedManager]showMessage:@"请填写快递公司"];
        return;
    }
    if (isEmpty(self.numField.text)) {
        [[BaseLoadingView sharedManager]showMessage:@"请填写快递单号"];
        return;
    }
