...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        self.contentHeight = kScreenHeight/2.4;
        self.barViewHeight = 55;
        self.btnWidth = 50;
...
- (UIView *)barView {
    if (!_barView) {
        _barView = [[UIView alloc] initWithFrame:CGRectMake(0, self.contentView.frame.size.height - self.barViewHeight, kWindowW, self.barViewHeight)];
...
- (UIView *)contentView {
    if (!_contentView) {
        _contentView = [[UIView alloc] initWithFrame:CGRectMake(0, kWindowH-30, kWindowW, self.contentHeight)];
...
- (UIPickerView *)pickerView {
    if (!_pickerView) {
        _pickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(0, 0, self.contentView.frame.size.width, self.contentView.frame.size.height - self.barViewHeight-15)];
...
- (UIButton *)comfirmBtn {
    if (!_comfirmBtn) {
        ...
        _comfirmBtn.frame = CGRectMake(14, -5, kScreenWidth-28, 40);
        ...
        _comfirmBtn.backgroundColor = KCOLOR_Main;
        _comfirmBtn.titleLabel.font=CUSTOMBOLDFONT(16);
        [_comfirmBtn addTarget:self action:@selector(comfirmBtnClick) forControlEvents:UIControlEventTouchUpInside];
        _comfirmBtn.layer.cornerRadius=20;
    }
    return _comfirmBtn;
}
- (UIButton *)cancelBtn {
