...
- (void)cellDidDeleteClcik:(UICollectionViewCell *)cell {
    if (!cell) {
        ...
        HXPhotoModel *model = self.dataList[indexPath.item];
    if(!isEmpty(model.networkPhotoUrl)||[model.videoURL.absoluteString containsString:@"mp4"]) {
        WEAK_SELF
        AlertViewManager *alertManager = [[AlertViewManager alloc]init];
        [alertManager showAlertViewWithControllerTitle:@"提示" message:@"是否删除此照片" controllerStyle:UIAlertControllerStyleAlert isHaveTextField:NO actionTitle:@"是" actionStyle:UIAlertActionStyleDefault alertViewAction:^(UIAlertAction *action) {
           
            [weak_self deleteShow:cell];
               
             
         } otherActionTitle:@"否" otherActionStyle:UIAlertActionStyleDefault otherAlertViewAction:^(UIAlertAction *action) {
                 
        }];
        
    }else{
          [self deleteShow:cell];
    }
...
-(void)deleteShow:(UICollectionViewCell *)cell{
