 ...
 *  修改某个产品的数量
 */
- (void)changeTheShopIndexPath:(NSIndexPath *)indexpath AndModify:(NSNumber *)count  AddIsSuccess:(AddAndSubtractBlock)block{
    ...
    WEAK_SELF
    [self requestAdjustShopCartGoodsData:model AndGoodsNum:count adjustShopCartBlock:^(BOOL isAdjust,NSString *status) {

        if (isAdjust) {

            model.goodsNumber = count;
            model.oldNumber= count;
            [weak_self GetTotalBill];
            emptyBlock(block);
        }else{
            model.goodsNumber = model.oldNumber;
            [weak_self GetTotalBill];

            emptyBlock(block);
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [[BaseLoadingView sharedManager]showMessage:status];
...
- (void)deleteSeletedArray:(ShopCartDelegateDataBlock)shopCartDelegateDataBlock{
    ...
    if (self.isSelecteAll ==YES) {
        ...
        [self requestDeleteShopCartGoodsForGoodsIds:arr deleteShopCartBlock:^(BOOL isDelete) {
            if (isDelete) {
