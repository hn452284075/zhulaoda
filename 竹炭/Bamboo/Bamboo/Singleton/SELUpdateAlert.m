...
/** 是否需要强制更新--app市场发布只要包含了更新两个字就强制更新，猥琐操作。不能及时控制 */
...
- (void)_setupUI
{
    ...
    updateButton.backgroundColor = KCOLOR_Main;
...
//    cancelButton.center = CGPointMake(CGRectGetMaxX(updateView.frame), CGRectGetMinY(updateView.frame));
    cancelButton.frame = CGRectMake(bgView.frame.size.width - Ratio(40)-12, 13, Ratio(36), Ratio(36));
    ...
    if([self.desc containsString:@"更新"])
    {
        NSLog(@"包含了只能去更新");
        self.isUpdate = YES;
    }else{
...
- (void)updateVersion
{
    NSString *str = [NSString stringWithFormat:@"http://itunes.apple.com/cn/app/id%@", APPSTORE_ID];
